...
> **⚠ ARCHIVED / NOT AUTHORITATIVE.** This file is not imported by any application code
> (verified by repo-wide search) and does not reflect the live site. The actual palette,
> type stack, and spacing/motion tokens are defined in `app/globals.css` — dark navy
> background (`--night: #050713`), teal signal accent (`--signal: #55d5d8`), all three
> font roles (`--font-ui`, `--font-editorial`, `--font-mono`) currently pointing at
> "Cascadia Code". The light/cyberpunk palette and Orbitron+JetBrains Mono pairing below
> are a stale generator scaffold — do not treat them as the current design direction, and
> do not "restore" this palette without deliberately deciding to replace the live one.
...
**Project:** Ahmed Asl Portfolio
**Generated:** 2026-07-21 03:52:34
