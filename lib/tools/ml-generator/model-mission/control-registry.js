...
function freezeControl(control) {
  return Object.freeze({
    ...control,
    taskIds: cloneAndFreeze(control.taskIds),
    defaultValue: cloneAndFreeze(control.defaultValue),
    explanation: cloneAndFreeze(control.explanation),
    ...(control.visibleWhen
      ? { visibleWhen: cloneAndFreeze(control.visibleWhen) }
      : {}),
    ...(control.enabledWhen
      ? { enabledWhen: cloneAndFreeze(control.enabledWhen) }
      : {}),
  });
}
...
export function evaluateRule(rule, project = {}) {
  ...
  if (rule.operator === "not-equals") return value !== rule.value;
...
export function validateMissionControlRegistry(
  controls = MODEL_MISSION_CONTROLS,
) {
  const errors = [];
  ...
  for (const control of controls) {
    for (const key of REQUIRED_CONTROL_KEYS) {
      const isMissingDefault = key === "defaultValue" && control[key] === undefined;
      if (isMissingDefault || (key !== "defaultValue" && (control[key] === undefined || control[key] === null || control[key] === ""))) {
