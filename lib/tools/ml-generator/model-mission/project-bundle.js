...
function artifactDetails(project, task) {
  ...
  if (task.id === "open-vocabulary-detection") {
    return {
      kind: "mission-entry",
      path: normalizedConfigPath(
        project,
        "output",
        "outputDirectory",
        "runs/open_vocabulary",
      ),
    };
  }

  if (task.id === "monocular-depth") {
    return {
      kind: "mission-entry",
      path: normalizedConfigPath(
        project,
  ...
  if (task.id === "semantic-segmentation") {
    return {
      kind: "mission-entry",
      path: normalizedConfigPath(
        project,
...
function missionEntryPrediction() {
  return `"""Run the generated mission with its configured inference example."""
...
function buildPrediction(project, artifact) {
  ...
  if (artifact.kind === "mission-entry") {
    return missionEntryPrediction();
  }
...
function predictionInputPath(project, artifact) {
  ...
  if (artifact.kind === "mission-entry") {
    return normalizedConfigPath(
      project,
      "data",
      project.data?.sampleImagePath ? "sampleImagePath" : "sourcePath",
      "sample.jpg",
    );
  }
  return normalizedConfigPath(
