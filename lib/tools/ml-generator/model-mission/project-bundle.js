...
Prediction loads \`${artifact.path}\`. Place the documented inference input at the path named by \`src/predict.py\`.
...
function buildDataGuide({ project, task, result }) {
...
function sensorPrediction(artifact) {
  if (artifact.kind === "sensor-onnx") {
    return `"""Validate and inspect the exported sensor ONNX artifact."""
...
def main() -> None:
    ...
    print("Validated ONNX artifact:", artifact_path)
...
function requireBundleInput({ result, project, task }) {
...
export function buildMissionProjectBundle({ result, project, task }) {
  requireBundleInput({ result, project, task });
  validateConfiguredPaths(project);
  const rootName = safeProjectName(project.output?.projectName);
  const artifact = artifactDetails(project, task);
  const files = {
    ".gitignore": gitignore(),
    ...
    "data/README.md": buildDataGuide({ project, task, result }),
