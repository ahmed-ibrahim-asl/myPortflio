...
const KNOWN_MULTICLASS_CLASSIFICATION_DATASETS = new Set(["iris", "wine"]);
...
function buildDatasetSection(config) {
  ...
  if (config.splitStrategy === "group") {
    ...
    lines.push(
      ...
      "    raise ValueError(f\"Time column {TIME_COLUMN!r} was not found.\")",
      "X[TIME_COLUMN] = pd.to_datetime(X[TIME_COLUMN], errors=\"raise\")",
...
function buildEvaluationFunction(config) {
  if (config.task === "classification") {
    return [
      "",
      `DECISION_THRESHOLD = ${config.decisionThreshold === null ? "None" : config.decisionThreshold}`,
      "",
...
function searchParameters(config) {
  if (config.model === "linear-regression") {
    return '{"model__fit_intercept": [True, False]}';
  }
...
export function generateClassicalScript(input = {}) {
  ...
  if (
    ...
    && KNOWN_MULTICLASS_CLASSIFICATION_DATASETS.has(config.dataset)
  ) {
    validationErrors.decisionThreshold = "Decision thresholds require a binary classification dataset.";
  }
