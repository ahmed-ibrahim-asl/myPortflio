...
const SPLIT_STRATEGIES = new Set(["train-validation-test"]);
...
const DATA_SOURCES = Object.freeze({
  "tabular-classification": new Set([
    "breast-cancer",
    "iris",
    "wine",
    "custom-csv",
  ]),
  "tabular-regression": new Set(["diabetes", "custom-csv"]),
  "image-classification": new Set([
    "image-folder",
    "custom-image-folder",
  ]),
  "sequence-classification": new Set([
    "sequence-array",
    "custom-npz",
  ]),
});
const BUILT_IN_CLASS_COUNTS = Object.freeze({
  "breast-cancer": 2,
  iris: 3,
  wine: 3,
});
...
const NEURAL_OPTION_LABELS = Object.freeze({
  framework: Object.freeze([
    { value: "keras", label: "Keras / TensorFlow" },
    { value: "pytorch", label: "PyTorch" },
  ]),
  splitStrategy: Object.freeze([
    {
      value: "train-validation-test",
      label: "Train + validation + final test",
    },
  ]),
  scaling: Object.freeze([
    { value: "none", label: "No learned scaling" },
...
const DATA_SOURCE_LABELS = Object.freeze({
  ...
  "custom-csv": "My CSV",
  "image-folder": "Image folders",
  "custom-image-folder": "Custom image folders",
  "sequence-array": "Sequence array",
  "custom-npz": "Custom NPZ sequence archive",
});

function selectedPreset(project = {}) {
  const presetId = project.model?.preset ?? project.preset ?? DEFAULTS.preset;
  return NEURAL_PRESETS.find(({ id }) => id === presetId)
    ?? NEURAL_PRESETS[0];
}

export function getNeuralControlOptions(controlId, project = {}) {
  const preset = selectedPreset(project);
  if (controlId === "preset") {
    return NEURAL_PRESETS.map(({ id: value, label }) => ({
      value,
      label,
    }));
  }
  if (controlId === "dataSource") {
    return [...DATA_SOURCES[preset.task]].map((value) => ({
      value,
      label: DATA_SOURCE_LABELS[value],
    }));
  }
  if (controlId === "scaling" && preset.task === "image-classification") {
    return [NEURAL_OPTION_LABELS.scaling[0]];
  }
  return NEURAL_OPTION_LABELS[controlId]
    ? [...NEURAL_OPTION_LABELS[controlId]]
    : [];
}
...
function getPreset(presetId) {
  const requestedId = presetId ?? DEFAULTS.preset;
  const preset = NEURAL_PRESETS.find(({ id }) => id === requestedId);
  if (!preset) {
    throw new NeuralConfigurationError(
      "architecture",
      `Unknown neural preset: ${requestedId}.`,
    );
  }
  return preset;
}
...
export function normalizeNeuralConfig(input = {}) {
  ...
  if (!DATA_SOURCES[task].has(dataSource)) {
    throw new NeuralConfigurationError(
      "data",
      `${dataSource || "The selected source"} is not compatible with ${task}.`,
    );
  }
  ...
  if (!SPLIT_STRATEGIES.has(splitStrategy)) {
    throw new NeuralConfigurationError(
      "split",
      "Neural training requires an explicit train, validation, and final-test split.",
    );
  }
  ...
  const validationRatio = requiredNumber(
  ...
  const requestedNumClasses = task === "tabular-regression"
    ? 1
    : BUILT_IN_CLASS_COUNTS[dataSource]
      ?? (input.numClasses === undefined
        ? DEFAULTS.numClasses
  ...
  const requestedLayers = Array.isArray(input.layers)
    ? input.layers
    : preset.layers;
  const scaling = dataContract === "image-folder"
    ? "none"
    : String(input.scaling ?? DEFAULTS.scaling);
  ...
  for (const layer of layers) {
    ...
    if (layer.type === "dense") {
      ...
      if (before.length !== 2) {
        ...
        after = [Math.floor(before[0] / layer.poolSize), before[1]];
      }
      ...
      if (before.length !== 3) {
        ...
        after = [
          Math.floor(before[0] / layer.poolSize),
          Math.floor(before[1] / layer.poolSize),
          before[2],
        ];
      }
...
function buildKerasTabularLoader(config) {
  ...
  return [
    ...
    "        \"input_shape\": tuple(np.asarray(X_train).shape[1:]),",
...
function buildKerasImageLoader() {
  return [
    ...
    "    }",
    "    return train_data, validation_data, test_data, preprocessing",
  ];
}
...
function buildKerasSequenceLoader(config) {
  ...
  return [
    ...
    "    y_train, y_validation, y_test, encoder = _encode_sequence_targets(",
...
function kerasDependencies(config) {
  if (config.dataContract === "tabular") {
    return ["joblib", "keras", "numpy", "pandas", "scikit-learn"];
  }
  if (config.dataContract === "sequence-array") {
    return ["joblib", "keras", "numpy", "scikit-learn"];
  }
  return ["joblib", "keras", "numpy"];
}
...
function buildKerasScript(config, inference) {
  ...
  return [
    ...
    "    train_data, validation_data, test_data, preprocessing = load_data()",
    ...(config.dataContract === "image-folder"
      ? ["    model = build_model()"]
...
function torchDependencies(config) {
  if (config.dataContract === "tabular") {
    return ["joblib", "numpy", "pandas", "scikit-learn", "torch"];
  }
  if (config.dataContract === "sequence-array") {
    return ["joblib", "numpy", "scikit-learn", "torch"];
  }
  return ["joblib", "numpy", "torch", "torchvision"];
}
...
function buildTorchImageLoader(config) {
  ...
  return [
    ...
    "        \"pixel_scale_denominator\": 255.0,",
    "    }",
    "    return train_base, validation_base, test_base, preprocessing",
  ];
}
...
function buildTorchScript(config, inference) {
  ...
  return [
    ...
    "    CHECKPOINT_PATH.parent.mkdir(parents=True, exist_ok=True)",
    "    ARTIFACT_PATH.parent.mkdir(parents=True, exist_ok=True)",
    "    train_dataset, validation_dataset, test_dataset, preprocessing = load_datasets()",
    ...
    "    )",
    "    joblib.dump(preprocessing, PREPROCESSING_PATH)",
    "    checkpoint[\"preprocessing_path\"] = str(PREPROCESSING_PATH)",
    "    checkpoint[\"class_names\"] = preprocessing.get(\"class_names\", [])",
