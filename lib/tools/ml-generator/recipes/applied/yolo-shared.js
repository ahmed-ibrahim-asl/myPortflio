...
const YOLO_MODEL_FAMILY_OPTIONS = [
  { value: "yolo26", label: "YOLO26 (current)" },
  { value: "yolo11", label: "YOLO11" },
  { value: "yolov8", label: "YOLOv8 (legacy compatible)" },
];
...
function resolveYoloWeights(modelFamily, modelSize, suffix = "") {
  const family = ["yolo26", "yolo11", "yolov8"].includes(modelFamily)
    ? modelFamily
    : "yolo26";
  const sizeCode = YOLO_SIZE_CODES[modelSize] ?? "n";
  return `${family}${sizeCode}${suffix}.pt`;
}
...
function createYoloFields({ defaultRunName, defaultProjectDirectory }) {
  return [
    ...
    },
    {
      ...
      inputType: "select",
      modes: ["starter", "production"],
      helpText: "Start with current YOLO26 or keep YOLO11/YOLOv8 for an existing project.",
      options: YOLO_MODEL_FAMILY_OPTIONS,
    },
    {
      id: "modelSize",
      label: "Model size",
    ...
    },
    {
      ...
      max: 1,
      step: 0.0001,
      visibleWhen: (config) => isTraining(config) && config.optimizer !== "auto",
    },
    {
      id: "optimizer",
      label: "Optimizer",
      inputType: "select",
      modes: ["production"],
      helpText: "Automatic selection chooses a compatible optimizer and learning rate.",
      options: YOLO_OPTIMIZER_OPTIONS,
      visibleWhen: isTraining,
    },
    {
      id: "validationConfidence",
      label: "Validation confidence",
      inputType: "number",
      modes: ["production"],
      helpText: "Minimum confidence retained while calculating validation metrics.",
      min: 0,
      max: 1,
      step: 0.01,
      visibleWhen: hasDataset,
    },
    {
      ...
      inputType: "number",
      modes: ["production"],
      helpText: "Minimum confidence retained in saved prediction results.",
      min: 0,
      max: 1,
      step: 0.01,
      visibleWhen: hasInference,
    },
    {
      ...
      inputType: "number",
      modes: ["production"],
      helpText: "Regularization strength applied while training.",
      min: 0,
      step: 0.0001,
      visibleWhen: isTraining,
    },
    {
      ...
      inputType: "number",
      modes: ["production"],
      helpText: "Momentum used by momentum-based optimizers.",
      min: 0,
      max: 1,
      step: 0.001,
      visibleWhen: isTraining,
    },
    {
      ...
      inputType: "number",
      modes: ["production"],
      helpText: "Training epochs used to ramp up optimization safely.",
      min: 0,
      step: 1,
      visibleWhen: isTraining,
    },
    {
      ...
      inputType: "number",
      modes: ["production"],
      helpText: "Number of leading layers kept fixed during fine-tuning.",
      min: 0,
      step: 1,
      visibleWhen: isTraining,
    },
    {
      ...
      inputType: "number",
      modes: ["production"],
      helpText: "IoU threshold used when removing overlapping detections.",
      min: 0,
      max: 1,
      step: 0.01,
      visibleWhen: (config) => hasDataset(config) || hasInference(config),
    },
    {
      id: "deterministic",
      label: "Deterministic training",
      inputType: "toggle",
      modes: ["production"],
      helpText: "Favor repeatable training behavior over some runtime performance.",
      visibleWhen: isTraining,
    },
    {
      id: "patience",
...
function normalizeYoloConfig(inputConfig, mode, defaults) {
  ...
  const config = { ...clone(modeDefaults), ...input };
  const legacyFamily = Object.keys(input).length > 0
    && !Object.hasOwn(input, "modelFamily")
    ? "yolov8"
    : modeDefaults.modelFamily;

  if (
    !Object.hasOwn(input, "predictionConfidence") &&
    Object.hasOwn(input, "confidenceThreshold")
  ) {
    config.predictionConfidence = input.confidenceThreshold;
  }
  delete config.confidenceThreshold;
  ...
  config.modelFamily = normalizeSelectValue(
    config.modelFamily,
    YOLO_MODEL_FAMILY_OPTIONS,
    legacyFamily,
  );
  if (!Object.hasOwn(input, "modelFamily") && Object.keys(input).length > 0) {
    config.modelFamily = legacyFamily;
  }
  ...
  config.optimizer = normalizeSelectValue(
    config.optimizer,
    YOLO_OPTIMIZER_OPTIONS,
    modeDefaults.optimizer,
  );
  ...
  config.deterministic = toBoolean(config.deterministic, modeDefaults.deterministic);
  ...
  if (!isTraining(config)) {
    ...
    config.optimizer = modeDefaults.optimizer;
    config.learningRate = modeDefaults.learningRate;
    config.weightDecay = modeDefaults.weightDecay;
    config.momentum = modeDefaults.momentum;
    config.warmupEpochs = modeDefaults.warmupEpochs;
    config.freezeLayers = modeDefaults.freezeLayers;
    config.deterministic = modeDefaults.deterministic;
...
function validateMinimum(errors, config, key, label, minimum, options = {}) {
  const value = config[key];
  if (!Number.isFinite(value)) {
    errors[key] = `${label} must be a number.`;
    return;
  ...
  if (value < minimum) {
    errors[key] = `${label} must be at least ${minimum}.`;
  }
}
...
function validateYoloConfig(config, mode) {
  ...
  if (resolvedMode === "production") {
    if (isTraining(config)) {
      ...
      validateMinimum(errors, config, "weightDecay", "Weight decay", 0);
      validateNumber(errors, config, "momentum", "Momentum", 0, 1);
      validateMinimum(errors, config, "warmupEpochs", "Warmup epochs", 0);
      validateMinimum(errors, config, "freezeLayers", "Freeze layers", 0, { integer: true });
    ...
    validateNumber(errors, config, "iouThreshold", "IoU threshold", 0, 1);
...
function getYoloWarnings(config, mode, kind) {
  ...
  if (isTraining(config) && config.optimizer === "auto") {
    warnings.push("Automatic optimizer selection chooses its own learning rate.");
  }
  if (
...
const YOLO_DETECTION_TEMPLATE = {
  ...
  shortDescription: "Train, validate, infer, and export a current YOLO26 detector, with YOLO11 and YOLOv8 compatibility.",
  ...
  generate(config, mode) {
    return generateYoloScript({
      ...
      mode,
      yoloTask: "detect",
      modelFilename: resolveYoloWeights(config.modelFamily, config.modelSize),
  ...
  notes: [
    "New missions use YOLO26; select YOLO11 or YOLOv8 when continuing an older project.",
...
CONFIG: dict[str, Any] = {
    ...
    "optimizer": ${pythonLiteral(config.optimizer)},
${config.optimizer === "auto" ? "" : `    "learning_rate": ${pythonLiteral(config.learningRate)},\n`}    "validation_confidence": ${pythonLiteral(config.validationConfidence)},
    "prediction_confidence": ${pythonLiteral(config.predictionConfidence)},
    "weight_decay": ${pythonLiteral(config.weightDecay)},
    "momentum": ${pythonLiteral(config.momentum)},
    "warmup_epochs": ${pythonLiteral(config.warmupEpochs)},
    "freeze_layers": ${pythonLiteral(config.freezeLayers)},
    "iou_threshold": ${pythonLiteral(config.iouThreshold)},
    "deterministic": ${pythonLiteral(config.deterministic)},
...
const YOLO_SEGMENTATION_TEMPLATE = {
  ...
  shortDescription: "Configure YOLO26 instance segmentation, with YOLO11 and YOLOv8 compatibility.",
  category: "Computer Vision",
  ...
  generate(config, mode) {
    return generateYoloScript({
      ...
      mode,
      yoloTask: "segment",
      modelFilename: resolveYoloWeights(
        config.modelFamily,
        config.modelSize,
        "-seg",
      ),
